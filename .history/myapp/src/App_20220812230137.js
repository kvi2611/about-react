...
//============================== useEffect: sử dụng để thực hiện gọi API, thao tác với dom trực tiếp, làm việc với time(setTimeout, setInterval) hoặc thêm xóa các event listeners ==================================//
function App() {
  const [count, setCount] = useState(0);
  ...
  return (
    <>
      ...
      <button onClick={() => setAction('users')} style={{background: 'red'}}>Get users</button>
      <button onClick={() => setAction('comments')} style={{background: 'yellow'}>Get comments</button>
